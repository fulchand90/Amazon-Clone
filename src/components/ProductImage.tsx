...
const ProductImage = ({ product }: { product: Product }) => {
  ...
  useEffect(() => {
    if (product && product?.images && product.images.length > 0) {
      ...
      setImgUrl(product.images);
    } else {
      console.log('No images available - conditions failed');
    }
  }, [product]);

  // ✅ Temporarily remove the early return to see what happens
  // if (!product || !product?.images || product.images.length === 0) {
  //   return (
  ...
  return (
    <div className="flex flex-start">
      <div>
        {/* ✅ Safe rendering with debug info */}
        <div className="mb-2 p-2 bg-red-100 text-xs">
          <p>Debug: Images array length: {product?.images?.length || 0}</p>
          <p>Debug: First image: {product?.images?.[0] || 'none'}</p>
        </div>
        ...
        {product?.images?.length > 0 ? (
          ...
          <div className="p-4 bg-yellow-100">
            <p>No images in array</p>
            <p>Product: {JSON.stringify(product, null, 2)}</p>
          </div>
        )}
      </div>
